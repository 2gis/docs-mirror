### 8.1. Перед установкой {#before-installing-navi}

1. Ознакомьтесь с:

   * [документацией сервисов навигации](/on-premise-api-platform/architecture/navigation);
...
2. Убедитесь, что выполнены предварительные шаги:
   ...
   5. (Опционально) [Установка прокси для API пробок](#install-traffic-proxy).
...
3. Соберите данные, заданные или полученные на предыдущих шагах:
   ...
   | Сервисные токены<sup>\*</sup>                          | `PUBLIC_TRANSPORT_TOKEN` <br /> `DISTANCE_MATRIX_TOKEN` <br /> `ISOCHRONE_TOKEN` <br /> `MAP_MATCHING_TOKEN` <br /> `TSP_TOKEN` <br />`DIRECTIONS_TOKEN` <br /> `TRUCK_DIRECTIONS_TOKEN` <br /> `PAIRS_DIRECTIONS_TOKEN` <br /> `ROUTES_PLANNER_TOKEN` (для работы с <Brand /> Ситискан) | См. [Установка сервиса API-ключей](/on-premise-api-platform/installation#install-keys)            |

   \* В иллюстративных целях предполагается, что сервисные токены доступны для всех продуктов навигации. Токены `DIRECTIONS_TOKEN`, `TRUCK_DIRECTIONS_TOKEN` и `PAIRS_DIRECTIONS_TOKEN` нужны для служебных Directions API, Truck Directions API и Pairs Directions API. 

4. Определите, какие API вам необходимо установить:

   * Базовые API навигации: Routing API, Directions API, Distance Matrix API, Map Matching API и Isochrone API. Подробнее о сервисах см. в [обзоре](/on-premise-api-platform/overview/summary#navigation).
   * Distance Matrix Async API для расчёта матрицы расстояний для большого количества точек. Может быть установлен отдельно или в сочетании с другими API. Подробнее о сервисе см. на [странице архитектуры](/on-premise-api-platform/architecture/navigation#async-distance-matrix-api).
   * TSP API для решения задачи коммивояжёра (построить кратчайший маршрут обхода точек). Подробнее о сервисе см. на [странице архитектуры](/on-premise-api-platform/architecture/navigation#tsp-api).
   * Restrictions API для управления собственной информацией о дорожных перекрытиях. Устанавливается в сочетании с другими API. Подробнее о сервисе см. на [странице архитектуры](/on-premise-api-platform/architecture/navigation#restrictions-api).
...
### 8.2. Подготовьте инфраструктуру {#prepare-infrastructure-navi}
...
### 8.3. Создайте файл правил {#rules-file}
...
| `routing` | Маршруты для транспорта, кроме общественного. | [Routing API](/api/navigation/routing/overview), [Directions API](/api/navigation/directions/overview) |
| `ctx`, `public_transport` | Маршруты для общественного транспорта. | [Routing API](/api/navigation/routing/overview) |
| `get_pairs` | Построение нескольких маршрутов одновременно, кроме маршрутов для общественного транспорта. | [Routing API](/api/navigation/routing/overview) |
...
### 8.4. Установите сервисы навигации {#install-navi-services}
...
1. Создайте конфигурационный файл для Helm. Подробное описание доступных параметров см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/navi-attractor/README.md). Присвойте файлу имя по схеме `values-attractor-<transport>.yaml` (например, `values-attractor-car.yaml`).
   ...
   * `kafka`: настройки доступа к брокеру Apache Kafka для взаимодействия с Distance Matrix Async API. Укажите, только если сущность Navi-Attractor используется для работы с Distance Matrix Async API.
     ...
     * `distanceMatrix`: названия топиков для взаимодействия с сервисом Distance Matrix Async API. Подробную схему взаимодействия см. в разделе [Архитектура сервисов навигации](/on-premise-api-platform/architecture/navigation#async-distance-matrix-api).
...
1. Создайте конфигурационный файл для Helm. Подробное описание доступных параметров см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/navi-back/README.md). Присвойте файлу имя по схеме `values-back-<service>.yaml` (например, `values-back-directions-car.yaml`).
   ...
   * `naviback`: настройки сервиса Navi-Back.
     ...
     * `indices`: подключение данных для скачивания файлов. Если вы получаете данные через [прокси для API пробок](/on-premise-api-platform/architecture/trafficproxy), параметры ниже обязательны и должны иметь значение `true`:
...
1. Создайте конфигурационный файл для Helm. Подробное описание доступных параметров см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/navi-router/README.md).
   ...
   * `keys`: настройки сервиса ключей. Если не задавать эти настройки, то проверка API-ключа для запроса будет пропущена.
     ...
     * `apis`: сервисные токены для сохранения статистики использования (см. [Сервис ключей](/on-premise-api-platform/architecture/keys)).
...
1. Создайте конфигурационный файл для Helm. Подробное описание доступных параметров см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/navi-async-matrix/README.md).
   ...
   * `kafka`: настройки доступа к брокеру Apache Kafka. Подробную схему взаимодействия сервисов через Apache Kafka см. в разделе [Архитектура сервисов навигации](/on-premise-api-platform/architecture/navigation#async-distance-matrix-api).
   ...
   * `keys`: настройки сервиса ключей.
     ...
     * `token`: сервисный токен (см. [Установка сервиса API-ключей](/on-premise-api-platform/installation#install-keys)).
...
1. Создайте конфигурационный файл для установки VRP Task Manager с помощью Helm. Подробное описание доступных параметров см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/navi-vrp-task-manager/README.md).
   ...
   * `dm`: настройки доступа к Distance Matrix Async API.
     ...
     * `key`: API-ключ для доступа к сервису. См. инструкцию [Управление доступом к API](/on-premise-api-platform/keys).
...
### 8.5. Проверьте работоспособность сервисов навигации {#test-navi}
...
2. Создайте файл `data.json` с телом запроса к API навигации. Примеры запросов вы можете найти в документации сервисов навигации:
   ...
   * [Routing API](/api/navigation/routing/overview): маршруты для общественного транспорта, автомобилей, такси, велосипедов, самокатов, мотоциклов, экстренных служб, грузового транспорта и пешеходов.
   * [Directions API](/api/navigation/directions/examples): маршруты для автомобилей, такси, велосипедов, самокатов, мотоциклов, экстренных служб и пешеходов (для использования мобильным SDK).
