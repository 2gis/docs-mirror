### 9.1. Перед установкой {#before-installing-pm}

1. Ознакомьтесь с:

   * [документацией Менеджера Платформы](/on-premise-api-platform/architecture/platform);
   * [архитектурой программного комплекса <Brand />](/on-premise-api-platform/architecture/solution-architecture);
   * [системными требованиями](/on-premise-api-platform/requirements).

2. Убедитесь, что выполнены предварительные шаги:

   1. [Подготовка к установке](#preparation).
   2. [Установка сервиса API-ключей](#install-keys).
   3. [Установка API для работы с картами](#install-maps).
   4. [Установка API для работы с поиском](#install-search).
   5. [Установка API для работы с навигацией](#install-navi).
...
3. Соберите данные, заданные или полученные на предыдущих шагах:
   ...
   | API-ключи                       | `MAPGL_KEY`<br />`CATALOG_KEY`<br />`NAVI_KEY`<br />`STATIC_KEY` | См. [Установка сервиса API-ключей](/on-premise-api-platform/installation#install-keys)                |
...
### 9.2. Установите Менеджер Платформы {#install-platform}
...
1. Создайте конфигурационный файл для Helm. Подробное описание доступных параметров см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/platform/README.md).
   ...
   ui:
    ...
    static:
      url: 'https://staticmaps.example.com'
      key: 'STATIC_KEY'
   ...
   * `ui`: базовые настройки приложения.
     ...
     * `pages`: список доступных страниц в приложении. Значения записываются одной строкой через запятую и без пробелов, например, `'status,playground'`. Первое значение в списке — страница по умолчанию. Минимально необходимые значения: `map_styles` (если вы устанавливаете Styles API), `status` и `playground`. Полный список возможных значений см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/platform/README.md).
      ...
      * `status`: перечень статусов для сервисов On-Premise.
        ...
        * `mapgl`: статус сервиса [MapGL JS API](/on-premise-api-platform/architecture/maps).
        * `search`: статус сервисов [Поиска](/on-premise-api-platform/architecture/search).
        * `navi`: статус сервисов [Навигации](/on-premise-api-platform/architecture/navigation).
        * `pro`: статус сервисов [<Brand /> Про](/on-premise-pro/architecture/pro-architecture).
        * `keys`: статус [Сервиса ключей](/on-premise-api-platform/architecture/keys).
        * `mapStyles`: статус [Styles API](/on-premise-api-platform/architecture/maps).

      * `platform`: настройки доступа к [сервису API-ключей](/on-premise-api-platform/architecture/keys):

        * `api.url`: хост сервиса в формате `HTTP(S)://HOST`.

      * `license`: настройки доступа к [сервису лицензий](/on-premise-api-platform/architecture/license):

        * `url`: хост сервиса в формате `HTTP://HOST`.

      * `mapgl`: настройки доступа к сервису [MapGL JS API](/on-premise-api-platform/architecture/maps):
      ...
      * `mapStyles`: настройки доступа к сервису [Styles API](/on-premise-api-platform/architecture/maps):

        * `url`: хост сервиса в формате `HTTP(S)://HOST`.
        * `key`: API-ключ для доступа к импорту стилей карт.

      * `catalog`: настройки доступа к сервису [Catalog APIs](/on-premise-api-platform/architecture/search):

        * `url`: хост сервиса в формате `HTTP(S)://HOST`.
        * `key`: API-ключ для сервиса.

      * `navi`: настройки доступа к сервису [API навигации](/on-premise-api-platform/architecture/navigation):

        * `url`: хост сервиса в формате `HTTP(S)://HOST`.
        * `key`: API-ключ для сервиса.

      * `static`: настройки доступа к сервису [Static API](/on-premise-api-platform/architecture/maps):
...
### 9.3. Проверьте работоспособность Менеджера Платформы {#test-pm}
...
В Менеджере Платформы вы можете проверить статус работы установленных сервисов, загрузить пользовательские стили и протестировать работу ключевых API: подробнее см. в инструкции [Работа с Менеджером Платформы](/on-premise-api-platform/user-guide/platform-manager).

### 9.4. Настройте аутентификацию пользователей {#authentication}
