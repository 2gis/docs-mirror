...
### 4.1. Перед установкой {#before-installing-stat}

1. Ознакомьтесь с:

   * [документацией сервиса сбора статистики](/on-premise-api-platform/architecture/statreceiver);
...
2. Убедитесь, что удовлетворены требования к ресурсам, приведённые в [Helm-чарте](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/stat-receiver/values.yaml). Подробнее о том, как это сделать, см. в разделе [Системные требования](/on-premise-api-platform/requirements).

   :::note[Используйте подходящий чарт]

   Содержание Helm-чарта, описанное в этом разделе, актуально для последней версии API-платформы (см. [Релизы API-платформы](/on-premise-api-platform/releases/api-platform)). Чтобы изучить параметры для предыдущих версий, откройте [values.yaml в GitHub](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/stat-receiver/values.yaml) и в списке тегов слева выберите тег `Platform-<версия>`.
...
### 4.2. Установите сервис сбора статистики {#install-stat-services}

#### Установите сервис Stat Receiver {#install-stat-receiver}
...
1. Создайте конфигурационный файл для Helm. Подробное описание доступных параметров см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/stat-receiver/README.md).
   ...
   * `kafka`: настройки Apache Kafka.
      ...
      * `servers`: адрес и порт кластера Apache Kafka, который был настроен на этапе [Установки сервиса API-ключей](/on-premise-api-platform/installation#configure-kafka).
   ...
   * `topicsPrefix`: префикс для имен топиков, которые будут созданы в Kafka. См. имя топика, которое вы задали в параметре `kafka.stats.topic` в [конфигурационном файле сервиса API-ключей](/on-premise-api-platform/installation#install-keys): префикс — начальная часть имени топика до `type.401` (фиксированная часть имени).
...
### 4.3. Проверьте работоспособность сервиса сбора статистики {#test-stat-collection-service}

После установки сервисов API-платформы вы сможете просматривать статистику их использования в [Менеджере Платформы](/on-premise-api-platform/user-guide/platform-manager#viewing-statistics) и настраивать лимиты в [веб-интерфейсе сервиса API-ключей](/on-premise-api-platform/keys).
