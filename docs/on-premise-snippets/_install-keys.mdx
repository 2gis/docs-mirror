### 3.1. Перед установкой {#before-installing-keys}

1. Ознакомьтесь с:

   * [документацией сервиса API-ключей](/on-premise-api-platform/architecture/keys);
...
3. Соберите данные, заданные или полученные на предыдущих шагах:
   ...
   | Путь к файлу манифеста                                          | `manifests/core/1640661259.json`         | См. [Получение артефактов установки](#artifacts) |

4. Убедитесь, что удовлетворены требования к ресурсам, приведённые в [Helm-чарте](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/keys/values.yaml). Подробнее о том, как это сделать, см. в разделе [Системные требования](/on-premise-api-platform/requirements).
...
### 3.2. Подготовьте инфраструктуру {#prepare-infrastructure-keys}

:::note[Настройка аналогов из реестра Минцифры]

Если вместо PostgreSQL, Apache Kafka и Redis вы используете [аналоги из реестра Минцифры](/on-premise-api-platform/requirements), для инструкции по их настройке обратитесь к официальной документации этих сервисов.
...
Если вы не планируете использовать [сервис сбора статистики](/on-premise-api-platform/architecture/statreceiver), пропустите этот шаг.
...
### 3.3. Установите сервис API-ключей {#install-keys-service}
...
1. Создайте конфигурационный файл для Helm. Подробное описание доступных параметров см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/keys/README.md).
   ...
   * `statApi`: настройки доступа к сервису Stat API.
     ...
     * `url`: ссылка на сервис. Обязательное значение, если для параметра `statApi.enabled` задано значение `true`.
...
### 3.4. Получите сервисные токены {#fetch-service-keys}
...
### 3.5. Проверьте работоспособность сервиса API-ключей {#test-keys-api}
...
2. Войдите в веб-интерфейс с учётными данными администратора — пользователя, которому назначена роль администратора с помощью утилиты `keysctl`. Вы должны увидеть [веб-интерфейс для управления API-ключами](/on-premise-api-platform/keys).
