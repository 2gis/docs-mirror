...
1. Создайте конфигурационный файл для Helm. Подробное описание доступных параметров см. [здесь](https://github.com/2gis/on-premise-helm-charts/blob/master/charts/staticmaps/README.md).
    ...
    * `keys`: настройки доступа к сервису API-ключей.
      ...
      * `token`: сервисный токен (см. [Установка сервиса API-ключей](/on-premise-api-platform/core/install/keys)). Обязательный параметр, если для параметра `app.access.enabled` указано значение `true`.
    ...
    * `license.url`: URL [сервиса лицензий](/on-premise-api-platform/architecture/core/license).
